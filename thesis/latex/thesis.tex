...
where $\Delta G_{conf}$ is the change in spring energy in going from bothbound to onebound, and $\Delta G_{bind}$ is the energy gained from binding to the microtubule, and A is an unknown constant known as the preexponential factor with units of $s^{-1}$. 
...
Rate constants govern the probability of transitioning between onebound and bothbound states. Rate constant
