...
\subsubsection{Transitioning states}
Transitions from one state to another are accomplished by calculating the rate of transition in a certain position, then using this rate divided by \verb|dt| as a probability of transfer at each timestep. The rates of transition are calculated by assuming they follow the Arrhenius rate equation $r = Ae^{-\Delta G/k_BT}$, where the preexponential factor is a parameter of the model and $\Delta G$ the energy of transition between the repsective states. A code snippet of this rate calculation is shown in Figure (\ref{transition-rate-snippet}).
...
\textit{todo: include lots more on the derivation of preexponential factors from experimental rates and loss of degrees of freedom}
...
Spring and Brownian forces are used to properly move the model through time. Appendices (\ref{onebound-motion-equatons}-\ref{bothbound-motion-equations}) show angular velocities are functions of the x- and y-components of both spring forces $F$ and Brownian forces $\gamma R$. We calculate these as shown:
...
\textbf{Possibly due mutations on motion equations to show that tests are no longer behaved}
...
The following are the full motion equations used to time evolve the onebound model:
...
Solving the above system for $\dot{\Theta}_{bb}, \dot{\Theta}_{bm}, \dot{\Theta}_{um}$ and $\dot{\Theta}_{ub}$ gives
...
\begin{itemize}
\item Crystal structure of dynein with rigid rods imposed and or a cartoon model
\item Brownian motion figure
\item Crystal structure transitioning from pre/post stroke
\item dt vs error plot
\item ET angle histogram
\item figure of finding lengths in PyMol
\item stepping length/time histograms
\end{itemize}
\end{document}
